(function () {
  ...
  function ready(w, h) {
    ...
    function draw() {
      ...
      each(flock, function (b) {
        fog(ctx, b.p.z);
        circle(ctx, flockBounds.x * b.p.x / b.p.z + flockBounds.y, flockBounds.x * b.p.y / b.p.z + flockBounds.x, mobile ? 30 / b.p.z : 60 / b.p.z);
      });
    ...
    var yMaxLine = max(lines, function (d) {
      return d.y;
    });
    var yMaxIndex = lines.findIndex(function (d) {
      return d.y === yMaxLine;
    });
    var yLowestLine = parseInt(flockBounds.x * lines[yMaxIndex].y / lines[yMaxIndex].z + flockBounds.x);

    // Position text.
    select('#mail').style('top', yLowestLine + 10 + 'px').transition().delay(250).style('opacity', 1);
  }
